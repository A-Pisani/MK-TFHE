                                   ...
                                   const LweKey *const key)
{
    ...
    // init b with noisy message
    sample->b = gaussian32(message, alpha);
    sample->current_variance = alpha*alpha;

#ifndef NDEBUG   // set other positions to zeros
    for (int i = 0; i < sample->parties; ++i)
    {
        if (i == p)
        {
#endif
            ...
            for (int j = 0; j < n; ++j)
            {
                ...
                sample->b += sample->a[p*n + j] * key->key[j];
            }
#ifndef NDEBUG
        }
        else
...
{
    const int32_t n = key->params->n;
    assert(p < sample->parties);

    // add own noise to b
    sample->b += gaussian32(0, alpha);
    sample->current_variance += alpha*alpha;
                                ...
                                const LweKey *const key)
{
    ...
    // add noise according to MKHE.PartDec (needed to prevent key leakage)
    sample->b += gaussian32(0, alpha);
    sample->current_variance += alpha*alpha;
}
...
#ifndef NDEBUG
    const int32_t parties = sample->parties;
...
// MK Bootstrapped FULL_ADDER_v2 
// Only the PK part of RLWEkey is used
EXPORT void full_adder_v2(MKLweSample *sum, const MKLweSample *x, const MKLweSample *y, const int32_t nb_bits,
                const MKLweBootstrappingKeyFFT_v2 *bkFFT, const LweParams* LWEparams, const LweParams *extractedLWEparams, 
                const TLweParams* RLWEparams, const MKTFHEParams *MKparams, const MKRLweKey *MKrlwekey)  {
    // carries
    ...
    for (int32_t i = 0; i < nb_bits; ++i) {
        //sumi = xi XOR yi XOR carry(i-1) 
        MKbootsXOR3(sum + i, x + i, y + i, carry, bkFFT, LWEparams, extractedLWEparams, RLWEparams,
            MKparams, MKrlwekey); 

        // carry = (xi AND yi) XOR (carry(i-1) AND (xi XOR yi))
        MKboots2OF3(carry + 1, x + i, y + i, carry, bkFFT, LWEparams, extractedLWEparams, RLWEparams,
            MKparams, MKrlwekey); 
