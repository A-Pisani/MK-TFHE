...
int32_t main(int32_t argc, char **argv) {
    ...
    for (int trial = 0; trial < nb_trials; ++trial)
    {
        ...
        // evaluate MK bootstrapped OR 
        cout << "Starting MK bootstrapped OR FFT version 2 method 2: trial " << trial << endl;
        clock_t begin_OR_v2m2 = clock();
        MKbootsOR_FFT_v2m2(test_out_v2m2, test_in1, test_in2, MKlweBK_FFT, LWEparams, extractedLWEparams, RLWEparams, MKparams, MKrlwekey);
        clock_t end_OR_v2m2 = clock();
        double time_OR_v2m2 = ((double) end_OR_v2m2 - begin_OR_v2m2)/CLOCKS_PER_SEC;
        cout << "Finished MK bootstrapped OR FFT v2m2" << endl;
        cout << "Time per MKbootOR_FFT gate v2m2 (seconds)... " << time_OR_v2m2 << endl;

        argv_time_OR_v2m2 += time_OR_v2m2;

        // verify OR
        int32_t outOR_v2m2 = MKbootsSymDecrypt(test_out_v2m2, MKlwekey);
        cout << "OR: clear = " << out << ", decrypted = " << outOR_v2m2 << endl;
        if (outOR_v2m2 != out) {
            error_count_v2m2 +=1;
    ...
    cout << "Average time per bootOR_FFT_v2m2: " << argv_time_OR_v2m2/nb_trials << " seconds" << endl;
