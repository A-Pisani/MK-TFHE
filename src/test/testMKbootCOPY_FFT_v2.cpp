...
int32_t main(int32_t argc, char **argv) {
    ...
    double argv_time_COPY_v2m2 = 0.0;
    ...
    for (int trial = 0; trial < nb_trials; ++trial)
    {
        ...
        // evaluate MK bootstrapped COPY 
        cout << "Starting MK bootstrapped COPY FFT version 2 method 2: trial " << trial << endl;
        clock_t begin_COPY_v2m2 = clock();
        MKbootsCOPY_FFT_v2m2(test_out_v2m2, test_in1, MKlweBK_FFT, LWEparams, extractedLWEparams, RLWEparams, MKparams, MKrlwekey);
        clock_t end_COPY_v2m2 = clock();
        double time_COPY_v2m2 = ((double) end_COPY_v2m2 - begin_COPY_v2m2)/CLOCKS_PER_SEC;
        cout << "Finished MK bootstrapped COPY FFT v2m2" << endl;
        cout << "Time per MKbootCOPY_FFT gate v2m2 (seconds)... " << time_COPY_v2m2 << endl;

        argv_time_COPY_v2m2 += time_COPY_v2m2;

        // verify COPY
        int32_t outCOPY_v2m2 = MKbootsSymDecrypt(test_out_v2m2, MKlwekey);
        cout << "COPY: clear = " << out << ", decrypted = " << outCOPY_v2m2 << endl;
        if (outCOPY_v2m2 != out) {
            error_count_v2m2 +=1;
    ...
    cout << "Average time per bootCOPY_FFT_v2m2: " << argv_time_COPY_v2m2/nb_trials << " seconds" << endl;
