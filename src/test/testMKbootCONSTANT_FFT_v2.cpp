...
int32_t main(int32_t argc, char **argv) {
    ...
    double argv_time_CONSTANT_v2m2 = 0.0;
    ...
    for (int trial = 0; trial < nb_trials; ++trial)
    {
        ...
        // evaluate MK bootstrapped CONSTANT 
        cout << "Starting MK bootstrapped CONSTANT FFT version 2 method 2: trial " << trial << endl;
        clock_t begin_CONSTANT_v2m2 = clock();
        MKbootsCONSTANT_FFT_v2m2(test_out_v2m2, mess1, MKlweBK_FFT, LWEparams, extractedLWEparams, RLWEparams, MKparams, MKrlwekey);
        clock_t end_CONSTANT_v2m2 = clock();
        double time_CONSTANT_v2m2 = ((double) end_CONSTANT_v2m2 - begin_CONSTANT_v2m2)/CLOCKS_PER_SEC;
        cout << "Finished MK bootstrapped CONSTANT FFT v2m2" << endl;
        cout << "Time per MKbootCONSTANT_FFT gate v2m2 (seconds)... " << time_CONSTANT_v2m2 << endl;

        argv_time_CONSTANT_v2m2 += time_CONSTANT_v2m2;

        // verify CONSTANT
        int32_t outCONSTANT_v2m2 = MKbootsSymDecrypt(test_out_v2m2, MKlwekey);
        cout << "CONSTANT: clear = " << out << ", decrypted = " << outCONSTANT_v2m2 << endl;
        if (outCONSTANT_v2m2 != out) {
            error_count_v2m2 +=1;
    ...
    cout << "ERROR v2m2: " << error_count_v2m2 << " over " << nb_trials << " tests!" << endl;
    cout << "Average time per bootCONSTANT_FFT_v2m2: " << argv_time_CONSTANT_v2m2/nb_trials << " seconds" << endl;
    cout << endl << "ERROR Encrypt/Decrypt: " << error_count_EncDec << " over " << nb_trials << " tests!" << endl;
