...
int32_t main(int32_t argc, char **argv) {
    ...
    for (int trial = 0; trial < nb_trials; ++trial)
    {
        ...
        // evaluate MK bootstrapped NOT 
        cout << "Starting MK bootstrapped NOT FFT version 2 method 2: trial " << trial << endl;
        clock_t begin_NOT_v2m2 = clock();
        MKbootsNOT_FFT_v2m2(test_out_v2m2, test_in1, MKlweBK_FFT, LWEparams, extractedLWEparams, RLWEparams, MKparams, MKrlwekey);
        clock_t end_NOT_v2m2 = clock();
        double time_NOT_v2m2 = ((double) end_NOT_v2m2 - begin_NOT_v2m2)/CLOCKS_PER_SEC;
        cout << "Finished MK bootstrapped NOT FFT v2m2" << endl;
        cout << "Time per MKbootNOT_FFT gate v2m2 (seconds)... " << time_NOT_v2m2 << endl;

        argv_time_NOT_v2m2 += time_NOT_v2m2;

        // verify NOT
        int32_t outNOT_v2m2 = MKbootsSymDecrypt(test_out_v2m2, MKlwekey);
        cout << "NOT: clear = " << out << ", decrypted = " << outNOT_v2m2 << endl;
        if (outNOT_v2m2 != out) {
            error_count_v2m2 +=1;
    ...
    cout << "Average time per bootNOT_FFT_v2m2: " << argv_time_NOT_v2m2/nb_trials << " seconds" << endl;
