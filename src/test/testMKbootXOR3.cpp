...
int32_t main(int32_t argc, char **argv) {
    ...
    for (int trial = 0; trial < nb_trials; ++trial)
    {
        ...
        int32_t out = (mess1 + mess2 + mess3) % 2 == 1 ? 1 : 0;
        ...
        cout << "Starting MK bootstrapped XOR FFT version 2 method 2: trial " << trial << endl;
        clock_t begin_XOR_v2m2 = clock();
        MKbootsXOR3(test_out_v2m2, test_in1, test_in2, test_in3, MKlweBK_FFT, LWEparams, extractedLWEparams, RLWEparams, MKparams, MKrlwekey);
        clock_t end_XOR_v2m2 = clock();
        double time_XOR_v2m2 = ((double) end_XOR_v2m2 - begin_XOR_v2m2)/CLOCKS_PER_SEC;
        cout << "Finished MK bootstrapped XOR FFT v2m2" << endl;
        cout << "Time per MKbootXOR_FFT gate v2m2 (seconds)... " << time_XOR_v2m2 << endl;

        argv_time_XOR_v2m2 += time_XOR_v2m2;

        // verify XOR
        int32_t outXOR_v2m2 = MKbootsSymDecrypt(test_out_v2m2, MKlwekey);
        cout << "XOR: clear = " << out << ", decrypted = " << outXOR_v2m2 << endl;
        if (outXOR_v2m2 != out) {
            error_count_v2m2 +=1;
    ...
    cout << "Average time per bootXOR_FFT_v2m2: " << argv_time_XOR_v2m2/nb_trials << " seconds" << endl;
