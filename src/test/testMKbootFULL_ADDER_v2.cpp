...
int32_t main(int32_t argc, char **argv) {
    ...
    double argv_time_full_adder_v2_v2m2 = 0.0;
     ...
     for (int trial = 0; trial < nb_trials; ++trial)
    {
        ...
        // evaluate MK bootstrapped full_adder_v2 
        //cout << "Starting MK bootstrapped full_adder_v2 FFT version 2 method 2: trial " << trial << endl;
        clock_t begin_full_adder_v2_v2m2 = clock();
        full_adder_v2(test_out_v2m2, test_in1, test_in2, nb_bits, MKlweBK_FFT, LWEparams, extractedLWEparams, RLWEparams, MKparams, MKrlwekey);
        clock_t end_full_adder_v2_v2m2 = clock();
        double time_full_adder_v2_v2m2 = ((double) end_full_adder_v2_v2m2 - begin_full_adder_v2_v2m2)/CLOCKS_PER_SEC;
        cout << "Finished MK bootstrapped full_adder_v2 FFT v2m2" << endl;
        cout << "Time per MKbootfull_adder_v2_FFT gate v2m2 (seconds)... " << time_full_adder_v2_v2m2 << endl;

        argv_time_full_adder_v2_v2m2 += time_full_adder_v2_v2m2;

        // verify full_adder_v2
        int32_t outfull_adder_v2_v2m2 = 0; //MKbootsSymDecrypt(test_out_v2m2, MKlwekey);

        for(int i=0; i< nb_bits + 1; i++){
            int ci = MKbootsSymDecrypt(&test_out_v2m2[i], MKlwekey)>0;
            outfull_adder_v2_v2m2 |= (ci<<i);
        }

        cout << "full_adder_v2: clear = " << out << ", decrypted = " << outfull_adder_v2_v2m2 << endl;
        if (outfull_adder_v2_v2m2 != out) {
            error_count_v2m2 +=1;
    ...
    cout << "Average time per bootfull_adder_v2_FFT_v2m2: " << argv_time_full_adder_v2_v2m2/nb_trials << " seconds" << endl;
